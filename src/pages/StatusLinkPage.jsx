...
// Mirrors RoomLinkPage.jsx exactly (same resolve → redirect → fallback mechanism) — see
// that file's comments for the overall approach. This resolves a public status id instead
// of a room invite code, via the backend's unauthenticated /status/:id/preview endpoint.
...
export default function StatusLinkPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/status/${encodeURIComponent(id)}/preview`)
      .then(r => r.ok ? r.json() : Promise.reject())
      .then(body => {
        const preview = body?.data;
        if (!preview?.id) return Promise.reject();
        setOwnerName(preview.ownerName ?? null);
        window.location.href = `twedot://status/${preview.id}`;
    ...
    <div style={{
      ...
      {status === 'opening' && (
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6 }}>
            {ownerName ? `Taking you to ${ownerName}'s post` : 'Taking you to the post'}
          </p>
        </div>
      )}
      ...
      {status === 'error' && (
        <div style={{ textAlign: 'center', maxWidth: 360 }}>
          ...
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: 40 }}>
            This post has expired or no longer exists.
          </p>
      ...
      {status === 'download' && (
        <div style={{ textAlign: 'center', maxWidth: 360 }}>
          ...
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: 40 }}>
            {ownerName ? `See ${ownerName}'s post and connect with people nearby.` : 'Chat, discover services, and connect with people nearby — all in one place.'}
          </p>
          ...
          <button
            onClick={() => id && (window.location.href = `twedot://status/${id}`)}
            style={{
