...
// Mirrors UserLinkPage.jsx exactly (same resolve → redirect → fallback mechanism) —
// see that file's comments for the overall approach. This resolves a room invite
// code instead of a user phone number/token.
...
export default function RoomLinkPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/rooms/invite/${encodeURIComponent(code)}`)
      ...
      .then(body => {
        ...
        window.location.href = `twedot://room/${room.id}`;
    ...
    <div style={{
      ...
      {status === 'opening' && (
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6 }}>
            {roomName ? `Taking you to "${roomName}"` : 'Taking you to the room'}
          </p>
        </div>
      )}
      ...
      {status === 'error' && (
        <div style={{ textAlign: 'center', maxWidth: 360 }}>
          ...
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: 40 }}>
            This room invite is invalid or has expired.
          </p>
      ...
      {status === 'download' && (
        <div style={{ textAlign: 'center', maxWidth: 360 }}>
          ...
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: 40 }}>
            {roomName ? `Join "${roomName}" and chat, discover services, and connect with people nearby.` : 'Chat, discover services, and connect with people nearby — all in one place.'}
          </p>
          ...
          <button
            onClick={() => resolvedRoomId && (window.location.href = `twedot://room/${resolvedRoomId}`)}
            style={{
