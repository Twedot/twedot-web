...
// Light neutral footer — matches the rest of the page's light theme instead
// of Bolt's own solid-black band. Uses the same CSS vars as everywhere else
// on the page (so it still tracks light/dark theme correctly) rather than
// the hardcoded white-on-dark colors this used to have.
export default function Footer() {
  return (
    <footer className="footer-wrap" style={{ background: 'var(--bg-alt)', borderTop: '1px solid var(--border-sub)', padding: '72px 64px 0', position: 'relative', zIndex: 1 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ borderTop: '1px solid var(--border-sub)', padding: '24px 0 36px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ color: 'var(--text-muted)', fontSize: 13, opacity: 0.7 }}>© 2026 Twedot Inc. All rights reserved.</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#22c55e' }} />
            <span style={{ color: 'var(--text-muted)', fontSize: 12, opacity: 0.8 }}>All systems operational</span>
          </div>
        </div>
      </div>
    </footer>
